...
export default class TabXuatBan extends Component {
    constructor(props) {
        super(props);
            ...
            let list = dataS.map((newJson, key) => {
                ...
                return (
                    <Layout key={key} style={Styles.flexFull}>
                        <TemplateGroupDuyet key={key} parDisplayDate={displayDate} parNewJson={newJson} parUserID={userId} parType={12} tabNavigation={navigation} parDuyet={false} />
                    </Layout>
                )
            })
    ...
    fetchData(valueDate) {
        ...
        var url = Config.BASE_URL + Config.API_LICHTUAN + 'LT_GetLichTuanNgayXuatBan';
        var data = {
            Ngay: realDate,
