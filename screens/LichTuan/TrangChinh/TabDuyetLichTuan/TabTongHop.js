...
export default class TabTongHop extends Component {
    constructor(props) {
        super(props);
            ...
            let list = dataS.map((newJson, key) => {
                ...
                return (
                    <Layout key={key} style={Styles.flexFull}>
                        <TemplateGroupDuyet key={key} parDisplayDate={displayDate} parNewJson={newJson} parUserID={userId} parType={2} tabNavigation={navigation} parDuyet={true} />
                    </Layout>
                )
            })
    ...
    fetchData(valueDate) {
        ...
        var url = Config.BASE_URL + Config.API_LICHTUAN + 'LT_GetLichTuanNgayTongHop';
        var data = {
            Ngay: realDate,
